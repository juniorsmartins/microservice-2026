package backend.finance.adapters.exceptions;

import backend.finance.application.exceptions.http400.EnterpriseValidationException;
import backend.finance.application.exceptions.http404.ResourceNotFoundCustomException;
import backend.finance.application.exceptions.http409.ResourceConflictRulesCustomException;
...
//                        objectError -> this.messageSource.getMessage(objectError, LocaleContextHolder.getLocale())));
    ...
    // ---------- 400 Bad Request ---------- //
    @ExceptionHandler(EnterpriseValidationException.class)
    public ResponseEntity<ProblemDetail> handleBadRequestCustom(EnterpriseValidationException ex, WebRequest webRequest) {
